...
jobs:
  build:
    ...
    steps:
      - uses: step-security/xcodebuild@v3
  ...
  build:
    ...
    steps:
      - uses: step-security/xcodebuild@v3
        with:
          swift: ${{ matrix.swift }}
          # ^^ step-security/xcodebuild selects the newest Xcode that provides the requested Swift
...
- GitHub list what is available for the current [14.8](https://github.com/actions/runner-images/blob/main/images/macos/macos-14-Readme.md)
  and [15](https://github.com/actions/runner-images/blob/main/images/macos/macos-15-Readme.md) images.
...
`step-security/xcodebuild` _will find that Xcode_ if you specify an appropriate value for
the `xcode` input.
